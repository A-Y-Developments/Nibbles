...
///
/// Failure to open the management URL is a **P2** event: the overlay stays
/// open and surfaces a transient toast (NOT an inline error block — that's
/// the P1 delete-account treatment).
...
class _CancelSubscriptionSheetState
  ...
  Future<void> _onContinue() async {
    ...
    // Capture the navigator BEFORE the await.
    ...
    if (ok) {
      ...
      // P2 failure — keep the sheet open so the user can retry. AppToast
      // renders via the root Overlay, so it paints above the modal sheet.
      AppToast.error(context, _kLaunchFailureCopy);
    }
  }
